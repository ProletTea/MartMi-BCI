...
MotorImagery.AllTime = str2double(get(handles.EditAllTime, 'String'));                      
MotorImagery.StageFlag = 0;                                                                
MotorImagery.Sample.Cnt = 0;                                                                    
MotorImagery.Sample.Stage1 = 0;
MotorImagery.Sample.Stage2 = 0;
MotorImagery.Sample.Stage3 = 0;
MotorImagery.Sample.Stage4 = 0;
MotorImagery.ImageFilePath = get(handles.EditImageFilePath, 'String');                      
...
MotorImagery.ImageConfigureFilePath = get(handles.EditImageConfigureFilePath, 'String');    
...
MotorImagery.AllEpoch = MotorImagery.numClass*MotorImagery.ClassEpoch;               
MotorImagery.RemainEpoch = MotorImagery.AllEpoch;                                           
MotorImagery.State = 'NOT Start';
MotorImagery.ParaImagery = [];                                              
...
sound(MotorImagery.ImageSound.SoundCue, MotorImagery.ImageSound.SoundFs);   
...
BufferSize = round(33*SampleRate*(IntervalTime.DispMs/1000));    
...
CircMask=zeros(m,n);  
CircMask(circle<=r*r)=1;  
CircMask(circle>r*r)=0;   
...
% ObjSerial.BytesAvailableFcn={@instrcallback, ZoomCoeff, Filter, DrawMap}; 
% ObjSerial.BytesAvailableFcnMode='terminator';
% ObjSerial.BytesAvailableFcnCount=10; 
% ObjSerial.BytesAvailableFcn={@EveBytesAvailableFcn,handles};

% handles.TimerHandle_TryStartCom = timer('TimerFcn',{@Timer_TryStartCom,handles},'ExecutionMode', 'fixedDelay', 'Period', 0.5);
    
try
    fopen(ObjSerial);
    set(handles.TextCurrentState, 'String', 'Open Serial Port Successfully!');
catch
    NumTryStartCom = NumTryStartCom+1;
    set(handles.TextCurrentState, 'String', ['Trying No.', num2str(NumTryStartCom), ' times']);
...
%     set(handles.TextCurrentState, 'String', 'å·²å¼€å§‹æŽ¥æ”¶æ•°æ?!');
% catch
%     set(handles.TextCurrentState, 'String', 'å‘é?æŽ¥æ”¶æŒ‡ä»¤å¤±è´?!');
% end
...
function mycom(hObject,eventdata,handles)   %è¿™æ˜¯ä¸²å£è¯»å–æ•°æ®åŽæ‰§è¡Œçš„callbackå‡½æ•°
tic;
%%%%%%%%%%%%%%%%%%%%%%%%%%%% Code By Guoyang Liu %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global Filter
global DrawMap
